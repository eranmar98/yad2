  ...
  return (
    <SafeAreaView edges={['top']} className="flex-1 bg-white">
      <FlatList
        ...
        ListHeaderComponent={
          <View className="mb-4">
            <Text className="text-center font-display text-2xl font-extrabold text-ink">מודעות</Text>
            {hasSearch && (
            ...
            {isRealEstate && (
              <View className="mt-5 rounded-2xl border border-ink/10 p-4">
                ...
                <Text className="mb-2 text-right font-sans text-xs font-bold text-ink/50">סוג עסקה</Text>
                <View className="mb-4 flex-row flex-wrap gap-2">
                  {dealTypeOptions.map((label) => (
                    <FilterChip
                      ...
                      onToggle={() => setSelectedDealTypes((prev) => toggleInSet(prev, label))}
                    />
                  ))}
                </View>
                <Text className="mb-2 text-right font-sans text-xs font-bold text-ink/50">סוג נכס</Text>
                <View className="flex-row flex-wrap gap-2">
                  {propertyTypeOptions.map((label) => (
                    <FilterChip
                      ...
                      onToggle={() => setSelectedPropertyTypes((prev) => toggleInSet(prev, label))}
                    />
                  ))}
                </View>
              </View>
        ...
        ListEmptyComponent={
          isLoading ? (
            ...
            <View className="mt-10 items-center">
              ...
              {hasSearch && (
                <Pressable
                  onPress={() => navigation.setParams({ category: undefined, q: undefined })}
                  className="mt-3"
                >
        ...
        renderItem={({ item }) => (
          <View className="flex-1 overflow-hidden rounded-2xl border border-ink/10">
            ...
            <View className="p-3">
              <Text className="text-right font-sans text-xs font-medium text-navy">{item.category}</Text>
              <Text numberOfLines={1} className="mt-1 text-right font-display text-sm font-bold text-ink">
                {item.title}
              </Text>
              <Text className="mt-1 text-right font-display text-base font-bold text-navy">
                {item.price} ₪
              </Text>
              <ContactSellerButton item={item} className="mt-3 w-full px-2 py-2" />
            </View>
          </View>
        )}
      />
